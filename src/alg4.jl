...
function boostNet(features, labels; m=2, delta=.5, gamma=1, T=10, B=10)
  ...
  net = Net2(1, zeros(T), zeros(T, d))  # init f0 = 0, b0 = 0
  ...
  for t = 1:T
    ...
    G = sum(alpha .* sigma(-labels .* sigma(w * features)))
...
type Net2
  t   # how many available
  b   # a column vector
  W   # w matrix, each row is a w
end

" output value "
evaluate(m::Net2, features) = sum(sigma(slice(m.W, 1:m.t, :) * features) .* m.b[1:m.t], 1)
